...
MKLPATH=/opt/intel/mkl/10.0.1.014/lib/32			#path to MKL library
MKLINCLUDE=/opt/intel/mkl/10.0.1.014/include		#path to MKL includes
...
ifort -L$MKLPATH -I$MKLINCLUDE -g -traceback -heap-arrays -o gain.out  ${way}fitting.f90 ${way}gain.f90 ${way}screening.f90 ${way}Fermi_level.f90 ${way}polarization1.f90 ${way}polarization_normal.f90 ${way}int_matrix.f90 ${way}exchange.f90 ${way}dielectrical_const.f90 ${way}E_field.f90 ${way}concentration.f90 ${way}polyint.f90 ${way}f_f.f90 -lmkl_intel -lmkl_intel_thread -lmkl_core  -lmkl_lapack -liomp5 -lpthread -L/usr/local/dislin/lib -ldislin -ldislnc
