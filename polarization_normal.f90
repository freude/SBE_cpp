subroutine polarization_normal(n_charact,k,l_k,Eh,Ee,mu,Ef_h,Ef_e,Tempr,conc,l_f,fff,V,exce,PSr)
...
!print *,'t=',j2
     do j1=1,size(k)
			...
			&sum(V(j1,:)*pp(j2-1,:)*k*stk))/h-damp*pp(j2-1,j1)/h!+sum((Co((1+(j1-1)*size(k)):(j1*size(k)))
	!+ij*Co1((1+(j1-1)*size(k)):(j1*size(k))))*pp(j2-1,:)*k*stk)
			...
			&sum(V(j1,:)*pp(j2-1,:)*k*stk))/h-damp*(pp(j2-1,j1)+stt*kk1/2)/h!+sum((Co((1+(j1-1)*size(k)):(j1*size(k)))
	!+ij*Co1((1+(j1-1)*size(k)):(j1*size(k))))*pp(j2-1,:)*k*stk)

			kk3=-ij*(omega(j1)-Eg/h-exce(j1)/h)*(pp(j2-1,j1)+stt*kk2/2)-ij*(ne(j1)-nh(j1))*&
			&(mu(j1)*E_field(t(j2-1)+stt/2)+&
			&sum(V(j1,:)*pp(j2-1,:)*k*stk))/h-damp*(pp(j2-1,j1)+stt*kk2/2)/h!+sum((Co((1+(j1-1)*size(k)):(j1*size(k)))
	!+ij*Co1((1+(j1-1)*size(k)):(j1*size(k))))*pp(j2-1,:)*k*stk)

			kk4=-ij*(omega(j1)-Eg/h-exce(j1)/h)*(pp(j2-1,j1)+stt*kk3)-ij*(ne(j1)-nh(j1))*&
			&(mu(j1)*E_field(t(j2-1)+stt)+&
			&sum(V(j1,:)*pp(j2-1,:)*k*stk))/h-damp*(pp(j2-1,j1)+stt*kk3)/h!+sum((Co((1+(j1-1)*size(k)):(j1*size(k)))
	!+ij*Co1((1+(j1-1)*size(k)):(j1*size(k))))*pp(j2-1,:)*k*stk)
...
end subroutine polarization_normal
